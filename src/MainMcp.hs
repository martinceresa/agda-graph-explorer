...
usage = unlines
  ...
  , "                        goal_brief, inspect, auto, construct, scratch, check,"
  , "                        give_file, new_module, lemmas, repair) backed by a live"
  , "                        `agda --interaction-json` session. Every mutator returns"
  , "                        a diff and writes only under write:true."
  , "                        Needs `agda` on $PATH (or --agda-bin)."
