...
data InspectEvent
  ...
  = EvTool  { evTool   :: !Text
    ...
    -- | A module @load@: the on-disk file body + its open goals. Drives the
    -- editing view.
  | EvGoals { evFile    :: !Text
            , evContent :: !Text
            , evGoals   :: ![GoalLite]
...
pageHtml = T.unlines
  ...
  , "  #feed{overflow:auto;padding:4px;}"
  ...
  , "  #goals{padding:4px 8px;border-bottom:1px solid #333;display:flex;flex-wrap:wrap;gap:4px 12px;}"
  , "  .goal{color:#ce9178;}"
  , "  #split{flex:1;display:grid;grid-template-rows:1fr auto;min-height:0;}"
  , "  #file{margin:0;overflow:auto;padding:4px 0;}"
  ...
  , "  #diff{margin:0;max-height:45%;overflow:auto;border-top:1px solid #333;background:#181818;}"
  , "  #diff div{white-space:pre;padding:0 8px;}"
  , "  #diff .add{color:#4ec9b0;background:#143b30;}"
  , "  #diff .del{color:#f48771;background:#3b1d18;}"
  ...
  , "  <section><h2>Editing &mdash; <span id='fname'>(no file loaded)</span></h2>"
  , "    <div id='goals'></div>"
  , "    <div id='split'><pre id='file'></pre><pre id='diff'></pre></div>"
  , "  </section>"
  , "</main>"
  , "<script>"
  ...
  , "  });"
  , "  function renderFile(content,hl){"
  , "    const lines=(content||'').split('\\n');"
  , "    fileBox.innerHTML=lines.map((ln,i)=>{"
  ...
  , "    }).join('');"
  , "  }"
  , "  function showGoals(ev){"
  , "    fname.textContent=ev.file; curContent=ev.content||'';"
  , "    renderFile(curContent,null); diffBox.innerHTML='';"
  , "    goalsBox.innerHTML=(ev.goals||[]).map(g=>"
  , "      `<span class='goal'>${esc(g.id)} : ${esc(g.type)}${g.line?(' @'+g.line+':'+g.col):''}</span>`).join('');"
  , "  }"
  , "  function showEdit(ev){"
  , "    if(ev.content){curContent=ev.content; fname.textContent=ev.file;}"
  , "    renderFile(curContent,diffRange(ev.diff)); renderDiff(ev.diff);"
  , "  }"
  ...
  , "    else if(ev.type==='goals')showGoals(ev);"
  , "    else if(ev.type==='edit')showEdit(ev);"
  , "  };"
  , "</script>"
  , "</body></html>"
  ]
