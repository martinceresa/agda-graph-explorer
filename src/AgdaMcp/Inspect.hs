...
pageHtml = T.unlines
  ...
  , "  function fmtTime(ts){if(!ts)return '';const d=new Date(ts);if(isNaN(d.getTime()))return ts.slice(11,19);const p=n=>String(n).padStart(2,'0');return p(d.getHours())+':'+p(d.getMinutes())+':'+p(d.getSeconds());}"
  , "  function addTool(ev){"
  ...
  , "        `<span class='t'>${esc(fmtTime(ev.ts))}</span>`+"
