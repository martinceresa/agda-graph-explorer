...
-- key that section accepts (read from the subcommand's 'flagSpecs', so the list
-- can never drift from the parser). Per-flag defaults are conveyed by each key's
...
defaultsYaml = unlines $
  ...
  where
    section (name, desc) =
      ...
      , "# " ++ name ++ " — " ++ desc
      , name ++ ":"
      ] ++ concatMap flagLines (subFlagPairs name)
    flagLines (key, help) =
      [ "  # " ++ help
      , "  # " ++ key ++ ":"
      ]
...
-- 'flagSpecs' (the same list that drives parsing / config / help), so the
-- @--show-defaults@ skeleton lists exactly the keys each section accepts.
subFlagPairs :: String -> [(String, String)]
subFlagPairs sub = case sub of
  "motif"          -> pairs Motif.flagSpecs
  "load-bearing"   -> pairs LoadBearing.flagSpecs
  "polyglot"       -> pairs Polyglot.flagSpecs
  "fingerprint"    -> pairs Fingerprint.flagSpecs
  "debt"           -> pairs Debt.flagSpecs
  "basket"         -> pairs Basket.flagSpecs
  "ledger"         -> pairs Ledger.flagSpecs
  "echo"           -> pairs Echo.flagSpecs
  "gravity"        -> pairs Gravity.flagSpecs
  "pyre"           -> pairs Pyre.flagSpecs
  "chokepoint"     -> pairs Chokepoint.flagSpecs
  "silhouette"     -> pairs Silhouette.flagSpecs
  "entwine"        -> pairs Entwine.flagSpecs
  "fiedler"        -> pairs Fiedler.flagSpecs
  "horizon"        -> pairs Horizon.flagSpecs
  "strata"         -> pairs Strata.flagSpecs
  "term-cluster"   -> pairs TermCluster.flagSpecs
  "concept-bundle" -> pairs ConceptBundle.flagSpecs
  "hint-bench"     -> pairs HintBench.flagSpecs
  _                -> []
  where
    pairs :: [FlagSpec o] -> [(String, String)]
    pairs specs = map (\s -> (flagName s, flagHelp s)) specs
