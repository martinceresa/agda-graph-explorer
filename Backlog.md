...
- **Migrate the four hand-rolled CLIs onto the shared `FlagSpec`** (agda-unused
  ...
  (`notes/UX.md` §1.4, a local working note): the stated acceptance gate — `--help` output
...
- **`graph-diff` — two-snapshot graph delta** (was Requests.md §2.3) — a
  ...
  impact growth), `--fail-on` CI policy gates. Full design + phased roadmap in
  the local working note `notes/Graph.Diff.Plan.md`. Reopen when the Arena CI gate
